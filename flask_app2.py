...
def add_entry():
    if datetime.today() < golive:
        ...
        cursor.execute('SELECT player_id, first_name, last_name, vegas FROM entries ORDER BY vegas ASC')
        golfer_data = cursor.fetchall()

        top20 = []
        not20 = []

        for player_id, first_name, last_name, vegas in golfer_data:
            full_name = f"{first_name} {last_name}"
            if vegas <= 20:
                top20.append(full_name)
            else:
                not20.append(full_name)
        not20.sort(key=lambda x: x[1])
        #top20.sort(key=lambda x: x[2])
        conn.close()
        ...
        if request.method == 'POST':
            ...
            selected_golfers = []
            for i in range(1, 7):
                golfer_name = request.form.get(f'golfer{i}')
                if golfer_name:
                    selected_golfers.append(golfer_name)
            print("Selected Golfers"+str(selected_golfers))
            if len(selected_golfers) != len(set(selected_golfers)):
                flash('Please select different golfers.')
            elif len(selected_golfers) != 6:
                flash('Please select exactly 6 golfers.')
            else:
                selected_players = selected_golfers
                #break
                #selected_players = [t[1] for t in selected_golfers]
                # selected_players = []
#                 for golfer_name in selected_golfers:
#                     for full_name in golfer_data:
#                         if full_name == golfer_name:
#                             selected_players.append((full_name))
#                             break
                print(selected_players)
                print(len(selected_players))
                if len(selected_players) != 6:
                    flash('Some selected golfers are not valid.')
                else:
                    conn = sql.connect('flask_app.db')
                    cursor = conn.cursor()
                    cursor.execute('INSERT INTO inputs2 (name, email, golfer1_id, golfer1_name, golfer2_id, golfer2_name, golfer3_id, golfer3_name, golfer4_id, golfer4_name, golfer5_id, golfer5_name, golfer6_id, golfer6_name, birdies) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                                   (name, email,
                                    selected_players[0][0], selected_players[0][1],
                                    selected_players[1][0], selected_players[1][1],
                                    selected_players[2][0], selected_players[2][1],
                                    selected_players[3][0], selected_players[3][1],
                                    selected_players[4][0], selected_players[4][1],
                                    selected_players[5][0], selected_players[5][1],
                                    birdies))
                    conn.commit()
                    conn.close()
